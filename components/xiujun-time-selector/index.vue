			...
			<block v-for="(item, _index) in timeArr" :key="_index">
				<view class="item">
					<view
						...
						:style="{ background:  timeActive.indexOf(_index)>=0 ? selectedItemColor : '#ffffff' }"
						@click="selectTimeEvent(_index, item)"
					>
						<text>{{ item.time }}</text>
						<text class="all" v-if="item.disable">{{ disableText }}</text>
						<text class="all" v-if="_index==timeActive[0]" style="font-size: 12upx;"></text>
						<text class="all" v-if="_index==timeActive[1]" style="font-size: 12upx;"></text>
					</view>
				</view>
			</block>
		</view>
