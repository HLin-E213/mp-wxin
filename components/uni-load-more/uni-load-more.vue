<template>
	<view class="uni-load-more" @click="onClick">
		...
		<!-- #endif -->
		<text class="uni-load-more__text" :style="{color: color}">{{ statusText }}</text>
	</view>
</template>
...
<script>
	...
	export default {
		...
		computed:{
		  statusText() {
		    const textMap = {
		      more: this.contentText.contentdown,
          loading: this.contentText.contentrefresh,
          nomore: this.contentText.contentnomore,
        };
