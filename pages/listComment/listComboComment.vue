...
<template>
  <view class="listComment" :style="isIPX? 'padding-bottom: 150rpx': ''">
    ...
    <view class="listComment_content">
      <view class="content_list" v-for="(i,k) in commentList" :key="k">
...
import {getOrderComboCommentList, getOrderCommentList} from '../../api/index';
...
export default {
  ...
  methods: {
    ...
    OrderCommentList: function (last = false) {
      let res = getOrderComboCommentList(this.info.id, this.page, this.pageSize), that = this;
      res.then(function (e) {
        // console.log(e);
        if (last) {
