      ...
      <view class="cmd-progress cmd-progress-default" :class="setStatusClass">
        <view class="cmd-progress-inner" :style="setCircleStyle">
          ...
          <svg viewBox="0 0 100 100" class="cmd-progress-circle">
            <path :d="setCirclePath" stroke="#FFC281" :stroke-linecap="strokeShape" :stroke-width="strokeWidth"
          ...
          <!-- 状态文本 start -->
          <block v-if="showInfo">
            <text class="cmd-progress-text" :class="{complete: setProgress == 100 }" :title="setFormat">
              <block v-if="status != 'exception' && setProgress <= 100">{{setFormat}}</block>
              <!-- #ifdef H5 -->
              ...
              <!-- #endif -->
              <!-- #ifndef H5 -->
              <text v-if="status == 'exception'" :style="setCircleIcon"></text>
              <!-- #endif -->
            </text>
          </block>
          <!-- 状态文本 end -->
...
<script>
   ...
   * @example <progress :percent="30"></progress>
   */
  export default {
    name: 'cmd-progress',
...
<style>
  ...
  .cmd-progress-text {
    ...
    color: #FFF3E8;
    font-size: 26rpx;
    text-shadow: 0 2px 2px rgba(64, 64, 64, 0.2);
  }
