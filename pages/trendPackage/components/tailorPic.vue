        ...
        methods:{
            ...
            updateUser: async function(val){
                try {
                    ...
                    const res = await updateUserImg(val);
                    const url = res.data.data.info[0].url;
                    await eventChannel.emit('accentPicFileFromTailorPic', { url });
                    uni.navigateBack({
                        delta: 1
                    });
                } finally {
