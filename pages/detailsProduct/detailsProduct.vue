...
<!--		    <view class="activity_bg" v-if="info.service_promotion_info.length == 0 ? false : true">-->
<!--		      <image class="festvial" src="https://admin.dajxyl.com/oss?path=img/festival.png" mode=""></image>-->
<!--		      <view class="downCount">-->
<!--		        <view class="surplus_count">-->
<!--		          <p>剩余数量</p>-->
<!--              <span>{{ info.service_promotion_info.num }}/{{ info.service_promotion_info.limit_number }}</span>-->
<!--		        </view>-->
<!--		        <view class="surplus_time" v-if="is_show_time">-->
<!--					<view class="surplus_time&#45;&#45;title">-->
<!--						<p v-if="countdown_type">活动距开始</p>-->
<!--						<p v-else>活动仅剩</p>-->
<!--					</view>-->
<!--					<view class="surplus_time&#45;&#45;content">-->
<!--						<b>{{ day1 }}</b>-->
<!--						<b>{{ day2 }}</b>-->
<!--						<b style="background: none;">天</b>-->
<!--						<b>{{ hour1 }}</b>-->
<!--						<b>{{ hour2 }}</b>-->
...
<!--		    </view>-->
		    <view class="activity_detail">
              ...
              <h4>¥ {{ present_price }}</h4>
              <p v-if="info.service_promotion_info.length == 0 ? false : true" class="price">指导价: ¥{{ original_price }}</p>
		        </view>
            <template v-if="info.service_promotion_info.length == 0 ? true : false">
              <view v-if="info.coupons.length > 0" class="activity_detail_top_right" @click="open">
                <span herf="#javascript">领取优惠券</span>
                <image src="https://admin.dajxyl.com/oss?path=img/fanhui3@2x_origin.png" mode=""></image>
              </view>
              <view v-else class="activity_detail_top_right" @click="openFail">
                <span herf="#javascript" style="color: #9A9A9A">领取优惠券</span>
                <image src="https://admin.dajxyl.com/oss?path=img/fanhuixiangqing@2x.png" mode=""></image>
              </view>
            </template>
            <template v-else>
              <view class="activity_detail_top_right">
                <span herf="#javascript" style="color: #9A9A9A"></span>
              </view>
            </template>
		      </view>
		      ...
		      <view class="activity_detail_bottom">
		        <view class="services_tag">
		          <view v-for="(tag,k) of info.homeTitle" :key="k" class="tag">
		            <p>{{ tag }}</p>
		          </view>
		        </view>
		      </view>
      ...
      <view class="evaluationUser" style="margin-bottom: 4.19rem;">
        ...
        <view
         class="evaluationUser_center"
          v-if="info.order_evaluation && info.order_evaluation[0]"
          >
          <image :src="info.order_evaluation[0].pic" mode=""></image>
          <span>{{ info.order_evaluation[0].nickname }}</span>
        </view>
        <view
            class="evaluationUser_bottom"
            v-if="info.order_evaluation[0]"
        >
...
import {codeInvitation, getServiceInfo, getUserInfo} from "../../api";
...
export default {
  ...
  onLoad(option) {
    ...
    if (option.id) {
      let that = this;
      let res = getServiceInfo(option.id);
      res.then(function(e){
  ...
  computed: {
    ...
    original_price: function() {
      if (this.productInfo.service_promotion_info.length !== 0) {
        return this.productInfo.present_price / 100;
      } else {
        return this.productInfo.original_price / 100;
      }
    },
    present_price: function() {
      console.log(this.productInfo);
      if (this.productInfo.service_promotion_info.length !== 0) {
        return this.productInfo.service_promotion_info.price / 100
      } else {
        return this.productInfo.present_price / 100
      }
    }
  ...
  created() {
  	// this.info.describe = this.info.describe.replace(/\<img/gi, '<img style="display:-webkit-box;width:355px !important;margin:10px auto !important;" ');
  	// console.log('info.coupons', this.info.coupons);
  	// console.log('info.coupon_text', this.info.coupon_text);
  },
  methods: {
    ...mapMutations('users', {
    ...
    onShareAppMessage: async function() {
      ...
      return {
        title: '大爱金孝',
        path: '/pages/detailsProduct/detailsProduct?id=' + this.info.id + '&code=' + code,
        success: (data) => {
          console.log(data)
        }
      }
    ...
    TimeCountdown: function () {
      ...
      //设置截止时间
      if (now > this.info.service_promotion_info.start_time * 1000) {
        var end = this.info.service_promotion_info.end_time * 1000;
        this.countdown_type = false;
      } else {
        var end = this.info.service_promotion_info.start_time * 1000;
        this.countdown_type = true;
      }
      ...
      var h, m, s, d;
      if (leftTime >= 0) {
        d = Math.floor(leftTime / 86400000);
      ...
      let hArr = h.toString().split("");
      let mArr = m.toString().split("");
      let sArr = s.toString().split("");
      // console.log(hArr,mArr,sArr);
      ...
      this.timer = setTimeout(this.TimeCountdown, 1000);
    },
    openFail: function(){
